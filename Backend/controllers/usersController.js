...
const getUser = async (req, res) => {
  ...
  if (!user) {
    return res
      .status(204)
      .json({ message: `No user matches ${req.param.name}` });
  }
  res.json(user);
};
...
const updateUserHistory = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      return res
        .status(404)
        .json({ message: `No user matches ${req.params.name}` });
    }
    ...
    const updatedUser = await User.findOne({ name: req.params.name }).populate(
      "recentProducts"
    );
